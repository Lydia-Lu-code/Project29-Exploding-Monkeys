...
//
//  Created by Lydia Lu on 2024/3/21.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        let first﻿Body: SKPhysicsBody
        let secondBody: SKPhysicsBody
        
        if contact.bodyA.categoryBitMask < contact.bodyB.categoryBitMask {
            first﻿Body = contact.bodyA
            secondBody = contact.bodyB
        } else {
            first﻿Body = contact.bodyB
            secondBody = contact.bodyA
        }
        
        guard let firstNode = first﻿Body.node else { return }
        guard let secondBody = secondBody.node else { return }
        
        if firstNode.name == "banana" && secondBody.name == "building" {
            bananaHit(building: secondBody, atPoint: contact.contactPoint)
        }
